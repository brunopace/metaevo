...
class Population():
    def __init__(self, EnvironmentSituation = None):
        self.time = 0
        self.generation = 0
        ...
        for o in range(Constants.population_size):
            self.population[o] = Organism(MetNet, inicial = True, ES = EnvironmentSituation)
    ...
    def step(self, index):   #contabiliza intermediarios baseado em Constants.intermenergy
        ...
        for o in range(Constants.population_size):
            if self.population[o] == None:
                continue
            ctrlstr = self.population[o].control.change_state() #inclui comidas, genes e intermediarios
            self.population[o].chemistry.update_reactions(self.population[o].control.switch_dict)
            enzyme_fraction = 0
            for j in range(len([g for g in self.population[o].control.nodes() if (g < Constants.metabolites + Constants.reactions if not Constants.intermenergy else True) and g >= Constants.food])):      #inclui interm ou nao dependendo de intermenergy
                enzyme_fraction+=ctrlstr[j + Constants.food]
    ...
    def divide(self, lista_o, rate, MetNet, ES):
        for o in lista_o:
            self.population.append(self.population[o].mutate(rate, MetNet, ES)) #nesse ponto avaliar o fitness do individuo novo
        ...
        fobj.write('\ncurrent ages:\n' + str([None if self.population[ind] == None else self.population[ind].age for ind in range(Constants.population_size)]))
    ...
    def genometofile(self):
        ...
        fobj.write('Thresholds: ' + '\n' + str([self.population[ix].control.node[n]['Threshold'] for n in self.population[ix].control.nodes()]) + '\n')
...
def constant_size(descriptive_string, environ_list, environ_change_method, period):
    ...
    a = Population(EnvironmentSituation)
    division = []
    chg = False
    env_ind = 0
    ...
    for my_step in xrange(Constants.end_step):          #INCLUIR UM METODO PARA SALVAR A POPULACAO NUM ARQUIVO QUANDO DA ENTRADA DO TECLADO!!!
