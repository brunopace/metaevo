...
\begin{frame}\frametitle{Coupling two Networks}
\begin{itemize}
 \item Two different directions: Signalling and Catalysis
 \item Gene regulatory network senses metabolites present in the environment and respond to them
\end{itemize}
\vspace{-10mm}
\makebox[\textwidth][c]{
...
\begin{frame}\frametitle{Population Dynamics}
\begin{itemize}
 \item Population dynamics and Evolution
 \item Biomass accumulation, enzyme fraction penalty and division threshold
 \item Fitness is implicitly defined
\end{itemize} 
\end{frame}

\section{Simplest Case}
\begin{frame}\frametitle{Learning AND function}
 \begin{itemize}
  \item In this setting, the selective pressure to learn depends on the environment
  \item Oscillating environment forces population to learn AND function
 \end{itemize}
\makebox[\textwidth][c]{
...
\begin{frame}\frametitle{Landscape}
 \begin{itemize}
  \item The landscape topology is defined, but the fitness of each genotype depends on the environment
  \item For an oscillatory environment, the best solution will be the AND function 
  \item The topology is a cartesian product between a Hamming graph and a path graph
 \end{itemize}
\makebox[\textwidth][c]{
...
\begin{frame}\frametitle{Genotype to Phenotype Mapping}
 \begin{itemize}
  \item Mapping also depends on environment - timescales
  \item For the oscillatory environment, $270$ different genotypes produce only $8$ different phenotype categories
 \end{itemize}
